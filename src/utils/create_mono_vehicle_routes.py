...
def create_mono_vehicle_routes(
    ...
    end_city_id: str,
    groceries_tons: int,
    find_path_function: Callable[[Map, str, str, Literal["land", "air", "sea"]], List[str]]
):
    capital_grocery_tons = map.get_city_by_id(capital_city_id)["capital_info"]["groceries_tons"]
    groceries_tons = min(groceries_tons, capital_grocery_tons)

    print("\n=====================")
    print(f"Finding best route from {capital_city_id} to {end_city_id} with {groceries_tons} tons of groceries.")
    ...
    if land_path and vehicles_used > 0:
        route = [Car(map, land_path[i], land_path[i + 1]) for i in range(len(land_path) - 1)]
        route_cost = sum_vehicles_cost(route, [vehicles_used] * len(route)).get_final_cost()
    ...
    if land_path and vehicles_used > 0:
        route = [Truck(map, land_path[i], land_path[i + 1]) for i in range(len(land_path) - 1)]
        route_cost = sum_vehicles_cost(route, [vehicles_used] * len(route)).get_final_cost()
    ...
    helicopters_available = map.get_city_by_id(capital_city_id)["capital_info"]["helicopters"]
    ...
    boats_available = map.get_city_by_id(capital_city_id)["harbor_info"]["boats"] if map.get_city_by_id(capital_city_id)["harbor_info"] else 0
    ...
    if sea_path and vehicles_used > 0:
        route = [Boat(map, sea_path[i], sea_path[i + 1]) for i in range(len(sea_path) - 1)]
        route_cost = sum_vehicles_cost(route, [vehicles_used] * len(route)).get_final_cost()
